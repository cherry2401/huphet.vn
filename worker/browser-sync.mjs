...
const minCollectionResponses = Number(process.env.WORKER_MIN_COLLECTION_RESPONSES ?? "3");
...
function getLaunchOptions() {
  return {
    headless,
    channel: browserPath ? undefined : process.env.WORKER_BROWSER_CHANNEL || undefined,
    ...
    viewport: {
      ...
      height: Number(process.env.WORKER_VIEWPORT_HEIGHT ?? "720"),
    },
    locale: "vi-VN",
...
function isBlockedUrl(url) {
  return url.includes("/buyer/login") || url.includes("/verify/traffic/error");
}

async function writeDiagnostics(name, payload) {
  await mkdir(diagnosticsDir, { recursive: true });
  const filePath = path.join(diagnosticsDir, `${name}.json`);
      ...
      flashSales.push({
        ...
        categoryId:
          typeof data?.flash_sale_session_tab?.global_fs_category_id === "number"
            ...
            : null,
        redirectUrl:
          typeof style?.redirect_url === "string" ? style.redirect_url : null,
      });
    }
  }
...
function normalizeDealItems(collectionResponses, collectionMap) {
  ...
  for (const response of collectionResponses) {
    ...
    const collectionName =
      collection?.componentName ?? collection?.name ?? `Collection ${collectionId}`;
    const items = response.payload?.data?.items ?? [];

    for (const item of items) {
      const itemData = item?.item_data ?? null;
      const collectionItemData = item?.collection_item_data ?? null;
      const displayedAsset = item?.customised_item_card?.item_card_displayed_asset ?? null;
      const displayPrice =
        itemData?.item_card_display_price ?? displayedAsset?.display_price ?? null;
      const itemId = item?.itemid ?? itemData?.itemid ?? collectionItemData?.item_id ?? null;
      const shopId = item?.shopid ?? itemData?.shopid ?? collectionItemData?.shop_id ?? null;
      const title =
        item?.name ??
        displayedAsset?.name ??
        collectionItemData?.display_name ??
        null;

      if (!itemId || !shopId || !title) {
        continue;
      }

      const salePrice = priceToNumber(
        item?.item_price ?? item?.price ?? displayPrice?.price ?? itemData?.price,
      );
      const originalPrice = priceToNumber(
        item?.price_before_discount ??
          item?.price ??
          displayPrice?.strikethrough_price ??
          displayPrice?.original_price ??
          itemData?.historical_lowest_price,
      );
      const discountPercent =
        originalPrice > salePrice && originalPrice > 0
          ? Math.max(0, Math.round(((originalPrice - salePrice) / originalPrice) * 100))
          : 0;
      const image = displayedAsset?.image ?? item?.image ?? null;
      ...
      deals.push({
        ...
        imageUrl: image ? `https://down-vn.img.susercontent.com/file/${image}` : undefined,
        shopId,
        itemId,
        collectionId,
        collectionName,
        ratingStar: itemData?.item_rating?.rating_star ?? displayedAsset?.rating ?? undefined,
      });
    }
  }
...
async function upsertCache(kind, data, slotKey, generatedAt) {
  ...
  if (error) {
    throw new Error(`Supabase upsert failed for ${kind}: ${error.message}`);
  }
}
...
async function main() {
  ...
  const parsedCookies = parseCookieHeader(process.env.SHOPEE_COOKIE);
  if (parsedCookies.length > 0) {
    await browserContext.addCookies(parsedCookies);
  }
  ...
  await page.route("**/api/v4/collection/get_items**", async (route, request) => {
    await route.continue({
      headers: buildProtectedHeaders(request.headers()),
    });
  });
  ...
  if (collectionResponses.length < minCollectionResponses) {
  ...
  const shouldWriteDeals = deals.length > 0;
  ...
  await page.evaluate(async () => {
    await new Promise((resolve) => {
      ...
      const timer = setInterval(() => {
        ...
        if (total >= document.body.scrollHeight * 0.8) {
          clearInterval(timer);
          resolve(undefined);
        }
      }, 300);
